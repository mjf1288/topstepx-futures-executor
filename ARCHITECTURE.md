...
|---|---|
| Contracts per instrument | `2` — counted from broker truth (positions + working entry orders, either side) |
| Contracts per account | `5` — 50K combine max position, summed across all symbols; **binding constraint**, since 2 × 5 symbols = 10 |
